import "./style.css";
import "../lib/worker.sql-wasm.js";
import simplify from "../lib/simplify.js";
//@ts-ignore
import L from "leaflet";
import "leaflet.heat";
import "https://cdn.jsdelivr.net/npm/react-leaflet-arrowheads@1.1.0/build/react-leaflet-arrowheads.min.js";
...
worker.onmessage = () => {
  ...
  worker.onmessage = (event) => {
    ...
    if (
      event.data.id == "put-averange-marker" &&
      document.querySelector<HTMLInputElement>("#avgmark")!.checked
    ) {
...
function onDoneStats() {
  ...
  worker.postMessage({
    ...
    action: "exec",
    sql:
      `SELECT MAX(altitude) AS maxalt, MIN(altitude) as minalt from TrackingPoint`,
  });
  ...
  worker.postMessage({
    ...
    action: "exec",
    sql:
      `SELECT sessionid, MAX(averageSpeed * 3.6) AS maxavgspeed FROM TrackingPoint`,
  });
...
function loadHeatmap(allPaths: allPathsType) {
  // @ts-ignore
  const latlngs = allPaths.flatMap((path) =>
    path.map(([lat, lng]) => L.latLng(lat, lng))
  );
  let heatmapEl = document.getElementById("heatmap")! as HTMLInputElement;
  const radius = document.getElementById("hradius")! as HTMLInputElement;
  let options = {
    blur: 15,
    gradient: {
      0.4: "blue",
...
function onDone() {
  let quality = document.querySelector<HTMLInputElement>("#quality")!.value;
  // let uColor = document.getElementById("unique-color").checked
  let opacity = false;
  // let opacity = document.getElementById("opacity").checked
  let arrows = document.querySelector<HTMLInputElement>("#arrows")!.checked;
...
document.querySelectorAll<HTMLInputElement>("input[name='view']").forEach(
  (el) => {
    el.oninput = () => {
      console.log("eej!");
      let selectedView =
